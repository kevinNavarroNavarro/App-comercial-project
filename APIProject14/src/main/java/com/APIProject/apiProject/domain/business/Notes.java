...
@Entity
@Table(name= "Notes", schema = "KADSupport")
public class Notes {
    @Id
    ...
    @JoinColumn(name = "SupportId", referencedColumnName="Id", nullable = false)
    private Supporter idSupport;

    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "SupervisorId", referencedColumnName="Id", nullable = false)
    private Supervisor idSupervisor;

    @NotNull
    @Column(name = "NoteTimestamp")
